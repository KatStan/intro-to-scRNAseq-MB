...
A Seurat object is a representation of single-cell expression data for R. Each Seurat object consists of one or more Assay objects aka representations of expression data (eg. RNA-seq, ATAC-seq, etc).
Each assay can have multiple layers. Each layer would be a transformed version of the raw gene expression data. These assays can be reduced from their high-dimensional state to a lower-dimension state and stored as ```DimReduc``` objects.
...
<!-- ToDo: Seurat deep-dive based on https://github.com/satijalab/seurat/wiki as collapsible -->
